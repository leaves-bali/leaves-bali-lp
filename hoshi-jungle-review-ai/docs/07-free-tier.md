...
2. AI の生成量に**月次の上限**を設ける（既定: 約 71 件/月）
...
|---|---|---|---|
| **`claude-haiku-4-5`（既定）** | $0.0056 | **約 71 件/月** | 約 892 件 |
| `claude-sonnet-5` | $0.0112 | 約 35 件/月 | 約 446 件 |
| `claude-opus-5` | $0.0280 | 約 14 件/月 | 約 178 件 |

> 数値は入力 1,600 tok / 出力 800 tok を安全側に見積もった概算です
> （`tests/pricing.test.ts` に回帰テストあり）。実績は `ai_usage` テーブルで確認できます。

**返信量を増やしたい場合**は `AI_MONTHLY_BUDGET_USD` を上げます。
月 $1.00 なら haiku で約 178 件/月、$5 クレジットは約 5 か月。
クレジットを使い切った後は、その額がそのまま月額費用になります
（月 71 件なら **約 $0.40 = 約 60 円/月**）。
