...
## Google以外のサイトのクチコミ（0012）

`reviews.source` に出どころを持たせました（`google` / `booking` / `expedia` /
`agoda` / `tripadvisor` / `trip_com` / `tabelog` / `other`）。

### なぜ貼り付けなのか

完全自動で返信できるのは **Google だけ**です。Agoda・トリップアドバイザー・
Trip.com・食べログは、返信を投稿するための API を外部に公開していません。
審査に通れば使えるという話ではなく、機能そのものが存在しません。
Booking.com と Expedia には API がありますが認定パートナー限定で、
Booking.com は新規の受付を止めています。

さらに**食べログは利用規約第 9 条で情報の自動収集を禁じています**。
したがって Google 以外は「人が貼り付け、AI が返信案を書き、人が貼り戻す」形です。
スクレイピングはしません。規約違反であり、画面変更のたびに壊れます。

### `google_review_id` を作り変えなかった理由

この列は `not null` かつ `unique` で、再同期時の冪等キーとして機能しています。
制約を緩めると Google 側の取り込みが壊れかねないため、貼り付けたクチコミには
`manual:<uuid>` という内部採番の ID を入れ、制約はそのまま活かしています。

### `replies.externally_replied_at`

Google 以外で「管理画面に貼り戻した」印です。`reply_status` の enum を増やすより、
時刻を 1 列足すほうが既存の判定を壊しません。

**これは押した記録であって、投稿されたことの保証ではありません。**
保証するには各サイトを読みに行くしかなく、規約上できません。
画面の文言もそれが伝わる書き方にしてあります。


## 月次の改善レポート（0013）

`monthly_reports` に、店舗ごと・月ごとのレポートを 1 行ずつ保存します。

```
monthly_reports
  location_id, period（対象月の1日）      … この2つで unique
  review_count / prev_review_count       … 件数と前月
  average_rating / prev_average_rating   … 平均評価と前月
  by_language                            … { "ja": 12, "en": 3 }
  reply_rate / prev_reply_rate           … 返信した割合（0.0〜1.0）と前月
  praised_themes / complained_themes     … [{ topic, count }]
  next_actions                           … 来月やること（最大3つ）
  model / generation_meta                … どのモデルで作ったか
  emailed_at                             … メール送信済みの記録
```

### なぜ保存するのか（その場で作らないのか）

毎回その場で生成すると、同じ月のレポートを開くたびに AI 費用が発生し、
文面も開くたびに変わります。「先月見たレポートと違うことが書いてある」は
信用を失います。1 回作って保存し、以後はそれを読みます。

`(location_id, period)` の unique 制約により、毎月 1 日のバッチが
再実行されても 1 行のままです（upsert で上書き）。

### 数字と文章を分けている理由

| 列 | 誰が作るか |
|---|---|
| `review_count` 〜 `prev_reply_rate` | **コードが数える**（`src/lib/reports/aggregate.ts`） |
| `praised_themes` / `complained_themes` / `next_actions` | AI |

件数・平均・返信率は事実であって文章ではありません。AI に数えさせると
桁を間違えたり、それらしい数字を作ったりします。営業資料（P.13）で
「導入前の1か月と比べてお伝えします」と約束している以上、ここが狂うと
約束そのものが嘘になります。

AI の出力も、そのままは保存しません（`src/lib/reports/themes.ts`）。

- 話題の件数は実際のクチコミ数を超えないように切る
- 1 件しか出ていない話題は傾向として扱わない（その月のクチコミが 5 件未満のときは残す）
- 言い換えの重複をまとめ、上位 5 件まで
- 「来月やること」は最大 3 つ。**足りなければ足りないまま出す**

### どの月のクチコミとして数えるか

投稿日（`google_create_time`）を優先し、無い場合だけ取り込み日（`created_at`）で
代用します。取り込み日を優先すると、初回同期の月に過去のクチコミが全部積み上がり、
「今月は 120 件」のような嘘の数字になります。

### 「返信した」の判定

次のいずれかなら返信済みとして数えます。

- `replies.published_at` がある（このシステムから公開した）
- `replies.externally_replied_at` がある（Google以外のサイトで貼り戻した）
- `reviews.has_google_reply` が true（**導入前に人が手で返していた分**）

3 つ目が重要です。これを落とすと「導入前の返信率 0%」という、
比較として意味のない数字が出ます。
