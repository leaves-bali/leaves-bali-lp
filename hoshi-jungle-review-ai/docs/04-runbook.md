...
`report_enabled` を契約している店舗に、**毎月 1 日 01:00 UTC**（日本時間 10:00）に
前月分のレポートを作ります。GitHub Actions の `Monthly Report` ワークフローです。

### なぜ GitHub Actions なのか

1 店舗でも AI 呼び出しが入るため、Netlify Free の関数（10 秒）では終わりません。
クチコミ同期と同じ理由です。月 1 回・数分なので無料枠（2,000 分）への影響は
ほぼありません。
...
| 平均評価（前月比） | 実データを数える |
| 言語ごとの件数 | 実データを数える |
| 返信できた割合（前月比） | 実データを数える |
| 褒められた点 / 指摘された点（件数つき） | AI |
| 来月やること（最大3つ） | AI |

画面（`/dashboard/report`）にも、どこまでが実測でどこからが AI かを明記しています。

### 手動で作り直す

GitHub の Actions タブ →「Monthly Report」→ `Run workflow` から、

- `period` … 対象月（`2026-08` の形。空なら前月）
- `force` … 作成済みでも作り直す

ローカルからでも動かせます。

```bash
REPORT_PERIOD=2026-08 REPORT_FORCE=true npm run report
```

### AI 費用

返信生成と**同じ月次予算**（`AI_MONTHLY_BUDGET_USD`）から出ます。
予算に達している月はレポートを作らず、見送った旨をログに出します。
入力は 1 店舗あたり最大 120 件（超えた分は低評価を優先して残す）・
1 件 400 文字までに切っているので、クチコミが多い店でも費用は一定です。

### メール送信（Resend）

レポートができると、店舗の連絡先メールアドレス（`locations.contact_email`、
未設定なら環境変数 `HOTEL_CONTACT_EMAIL`）宛に送ります。

**なぜ Resend か**: 無料枠が月3,000通・1日100通。月1回・店舗数ぶんの送信なら
当分無料で収まります。SendGrid は無料枠が廃止され（月$19.95〜）、
Amazon SES はサンドボックス解除の申請が要ります。
月0ドル運用を壊さずにいちばん早く動かせるのが Resend でした。

**なぜ SDK を入れないのか**: 使うのは「1通送る」だけで、
Resend の API はエンドポイント1つ・JSON 1つです。依存を増やすと保守が増えます。

#### 設定（これをしないとメールは送られません）

1. https://resend.com に登録
2. **Domains** で自社ドメインを追加し、表示された DNS レコードを登録する
   （認証していないドメインからは送れません）
3. **API Keys** で鍵を作る
4. GitHub → Settings → Secrets and variables → Actions
   - **Secrets** に `RESEND_API_KEY`
   - **Variables** に `REPORT_EMAIL_FROM`（例: `Leaves Bali <report@leavesbali.com>`）

#### 未設定でも止まりません

鍵が無いときはレポートだけ作り、メールは送りません。
**そのとき `emailed_at` は更新しません。**
ここを埋めてしまうと「送信済みなのに届かない」が起き、原因を追えなくなります。

送信に失敗した場合も例外は投げません。メールが送れないことはレポートが
作れないことより軽く、ここで止めると次の店舗のレポートまで作られなくなるためです。
失敗の理由（ドメイン未認証・宛先不正など）は Actions のログに出ます。

#### 文面

`src/lib/reports/renderEmail.ts` が組み立てます。通信を伴わない純粋な関数なので
テストで固定してあります（`tests/reportEmail.test.ts`）。
メールは送ったら取り消せないため、数字の欠落や記号によるレイアウト崩れを
テストで止めています。

Gmail は `<style>` の多くを落とすので、CSS はすべてインラインで、
レイアウトは `<table>` で組んでいます。古い作法ではなく、現在も必要な制約です。
HTML を表示しない環境向けのテキスト版も必ず付けています（無いと迷惑メール判定されやすい）。
