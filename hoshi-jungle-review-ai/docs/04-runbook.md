...
送信サービスを決めていないため、**差し込み口だけ用意してあります**
（`src/lib/reports/email.ts` の `sendMonthlyReportEmail`）。
現状は何も送らず、`emailed_at` も更新しません。
「送ったことになっているのに届かない」を避けるための作りです。

サービスを選ぶときの判断材料:

| サービス | 無料枠 | 備考 |
|---|---|---|
| Resend | 月3,000通・1日100通 | 設定が最も短い。独自ドメイン推奨 |
| Amazon SES | 月62,000通まで実質無料 | サンドボックス解除の申請が要る |
| SendGrid | **無料枠は廃止** | 現在は月$19.95〜 |

決まったら `sendMonthlyReportEmail` の中身を書くだけで、呼び出し側は変わりません。
